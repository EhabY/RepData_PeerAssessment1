...
```{r timeSeries1, fig.align="center"}
...
The new mean of the total steps per day is ```r format(round(new_totStepsMean))``` steps while the new median is ```r format(round(new_totStepsMedian))``` steps. We can see that both averages have gone up since the *NA* values have gone from zero to some positive number.
...
```{r timeSeries2, fig.align="center", message=FALSE}
grouped_df <- new_df %>%
    group_by(interval, day) %>%
